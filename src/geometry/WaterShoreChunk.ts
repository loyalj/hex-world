 ...
 *      This puts the foam material on the inner face of the shore hex.
...
export function buildShoreGeometry(
  ...
  const addVert = (x: number, z: number, u: number, v: number) => {
  ...
  const addTri = (
    x0: number, z0: number, u0: number, v0: number,
    x1: number, z1: number, u1: number, v1: number,
    x2: number, z2: number, u2: number, v2: number,
  ) => { addVert(x0, z0, u0, v0); addVert(x1, z1, u1, v1); addVert(x2, z2, u2, v2); };

  const addQuad = (
    x0: number, z0: number, u0: number, v0: number,
    x1: number, z1: number, u1: number, v1: number,
    x2: number, z2: number, u2: number, v2: number,
    x3: number, z3: number, u3: number, v3: number,
  ) => {
    addTri(x0, z0, u0, v0,  x1, z1, u1, v1,  x3, z3, u3, v3);
    addTri(x0, z0, u0, v0,  x3, z3, u3, v3,  x2, z2, u2, v2);
  };
  ...
  for (let row = rowStart; row < rowEnd; row++) {
    for (let col = colStart; col < colEnd; col++) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        // Water-side corners at WATER_FACTOR (V=0).
        const c1 = cornerAt(center.x, center.z, i,  WATER_FACTOR);
        const c2 = cornerAt(center.x, center.z, i1, WATER_FACTOR);

        // 1. Fan triangle — center → water corners, all V=0.
        //    Puts the foam material on the inner face of the hex for this edge.
        addTri(
        ...
        // 2. Strip quad — water corners → land solid corners.
        //    Skipped for estuary edges; EstuaryChunk handles those.
        const isEstuary = map.hasRiverThroughEdge(col, row, i);
        if (!isEstuary) {
          // Land-side corners: neighbor's solid corners facing back at us (V=1).
          //   current corner i  ↔ neighbor corner (i+4)%6
          //   current corner i1 ↔ neighbor corner (i+3)%6
        ...
        if (map.inBounds(nb2c, nb2r)) {
          ...
          // Corner of nb2 coinciding with current cell's corner i1: (i+5)%6 from nb2
          const cornerJ = (i + 5) % 6;
          ...
          // Determine the land-side of the CURRENT edge's ls2 for the corner vertex.
          // If the current edge is an estuary, we still need ls2 for the corner tri.
