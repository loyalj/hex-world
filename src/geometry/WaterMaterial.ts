...
const fragmentShader = /* glsl */`
  ...
  void main() {
    // Two layers scrolled along V (flow direction for rivers, world-Z for standing water).
    // Slightly different speeds produce a moiré-like ripple pattern.
    vec2 uv1 = vUv + vec2( uTime * 0.005, -uTime * 0.25);
    vec2 uv2 = vUv + vec2(-uTime * 0.0052, -uTime * 0.23);
