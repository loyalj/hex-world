...
- [x] Dirty rebuild creates water/shore/estuary meshes on demand — `waterChunks`, `shoreChunks`, `estuaryChunks` now get `else if` creation branches in the dirty rebuild loop matching `loadChunk`, so painting water onto an all-land chunk produces visible geometry without requiring a dispose/reload cycle
...
- [x] **Modular liquid surface types** — `LiquidTypeDescriptor` (`src/geometry/LiquidTypes.ts`): serializable asset (id, name, per-type geometry overrides: noiseScale, perturbStrength, surfaceLift); `LiquidMaterialSet` groups the four render materials (surface, shore, estuary, river) per liquid type; `DEFAULT_LIQUID_DESCRIPTORS` ships `water`, `lava`, `acid`; `TerrainDescriptor.liquidType?: string` replaces legacy `isWater` flag — any terrain referencing a liquid ID receives that liquid's geometry; `buildLiquidTerrainSets` produces a `Map<string, Set<number>>` used by `ChunkManager` to build one set of meshes per liquid type per chunk; `ChunkManagerOptions.liquidMaterials: Map<string, LiquidMaterialSet>` and `liquidDescriptors` replace the old four individual material slots (hard break); all four shader materials (`WaterMaterial`, `WaterShoreMaterial`, `EstuaryMaterial`, `RiverMaterial`) accept `LiquidColorOptions { shallow?, deep?, foam? }` so lava and acid get distinct colors; `MapSerializer` saves `liquidDescriptors?` in the JSON envelope; demo registers `LiquidShowcasePlugin` showing water ocean, lava caldera, and acid pools on one map
