...
- **Roads** — geometry strips rendered above terrain, lit and shadowed with the ground they sit on (day/night, cloud shadows, and sun shadows all reach them). Where a road reaches a river from both banks the cell gets a **bridge**: an arched road deck over the channel on a stone slab, and `hasBridge` tells your movement rules the same cell is dry
- **Scatter features** — instanced meshes (trees, rocks, buildings) placed deterministically from per-cell density levels; modular definitions with terrain filters, elevation and shore rules, per-definition density curves, and density tiers. Shapes are **data**: a recipe of primitive parts (bent trunks, drooping fronds, radial repeats) with a material descriptor for wind, seasons, snow, and texture, so a plant composed in the editor's Scatter Builder travels in a save or a `.hexpack` and comes back swaying
...
- **Procedural generation** — full climate-driven pipeline (BFS landmass → mountain ranges → erosion → moisture simulation → temperature model → biome assignment → volcanoes → rivers → roads), plus a fast FBM alternative; volcanoes are caldera pools ringed by a rim, ash-aproned, with a smoke scatter layer, and rivers end in the pool rather than carving through it; composable raw passes for custom generators; async time-sliced variants with progress events and cancellation for loading bars
...
- **Pathfinding** — A\*, flow fields (one Dijkstra sweep from the destination serves a whole army, with blended steering vectors so a crowd spreads instead of filing through one hex), flood-fill movement range, BFS visibility radius, elevation-aware line of sight, Catmull-Rom path smoothing; **movement domains** (`createDomainCost`) write land, naval, and amphibious rules once for every search — ships keep to the water except to dock at **port** cells, amphibious units pay to cross the shoreline
- **Units** — position, smooth path-following, facing, fog reveal, and shoreline transitions (`onEmbark` / `onDisembark`, with ships riding the water surface); wire your own `Object3D` and animation callbacks
...
npm run test:visual         # render fixed scenes headlessly and diff against the goldens
npm run test:visual:update  # re-bake the goldens after an intentional visual change
```
